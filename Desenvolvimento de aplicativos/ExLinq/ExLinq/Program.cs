...
static class Pesquisador
{
    ...
    {
        var departamentos = uni.Departamentos;
        var disciplinas = uni.Disciplinas;
        var result = departamentos
            .Join(disciplinas, x => x.ID, y => y.DepartamentoID,
            (x, y) => new
            {
                ID = x.ID,
    ...
    {
        var result = uni.Alunos.Select(x => new
        {
            Nome = x.Nome,
            Professor = x.TurmasMatriculados
            .Join
            (
                uni.Turmas,
                g => g,
                h => h.ID,
                (g, h) => new { h.ProfessorID }
             )
            .Join(
                uni.Professores,
                g => g.ProfessorID,// do que o query das turmas me passou
                h => h.ID, // do Uni.Professores
                (g, h) => new { h.Nome }
                )
        ...
        {
            Console.Write($"\n\n Aluno: {item.Nome}\nProfessores:");
            foreach (var turmas in item.Professor)
            {
                Console.Write($"{turmas.Nome};");
            }
        }
    }
    ...
    {
        var matriculas = uni.Alunos.SelectMany(
        ...
        var teste = uni.Professores
            .Join(
            uni.Turmas,
            g => g.ID,
            h => h.ProfessorID,
            (g, h) => new { IDTurma = h.ID, Professor = g.Nome, Salario = g.Salario }
            );
        ...
        var result = matriculas
            .Join(
                ...
                g => g.Turma,
                h => h.IDTurma,
                (g, h) => new { h.Professor, g.Aluno, h.Salario }
                );

        var result2 = result.GroupBy(x => x.Professor)
            .Select(x => new
            {
                Professor = x.Key,
                Salario = x.First().Salario,
                QtdAlunos = x.Count()
            });

        foreach (var item in result2)
        {
            Console.WriteLine($"O professor {item.Professor} Tem {item.QtdAlunos} alunos e recebe R${item.Salario}");
        }
    }
    ...
    public static void Pesquisa4(this Universidade uni)
    {
        ...
        result2 = result2.OrderByDescending(x => x.QtdAlunos).Take(10);

        foreach (var item in result2)
        {
            Console.WriteLine($"O professor {item.Professor} Tem {item.QtdAlunos}");
        }
    }
    ...
    public static void Pesquisa5(this Universidade uni)
    {
        ...
        var AlunosporTurma = uni.Alunos.SelectMany(
            a => a.TurmasMatriculados.Select(x => new
            {
                Aluno = a.Nome,
                Turma = x,
            })).GroupBy(x => x.Turma).Select(x => new
            {
                Turma = x.Key,
                QtdAluno = x.Count(), // tenho q achar o ID do professor 

            });

        var TurmaProfessor = uni.Turmas.Select(x => new
        {
            ProfessorId = x.ProfessorID,
            TurmaId = x.ID
        });

        var SalarioPorCabeca = AlunosporTurma
            .Join(TurmaProfessor,
            g => g.Turma,
            h => h.TurmaId,
            (g, h) => new
            ...
            //g => g.ProfessorID,
            //h => h.ProfessorId,
            //(g,h) => new {Valor = h.Salario/g.QtdAlunos, Nome = h.Nome}
            //).GroupBy(x => new { nome = x.Nome, Valor = x.Valor} );
